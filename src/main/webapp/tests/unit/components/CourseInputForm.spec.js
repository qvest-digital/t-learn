...
describe("CourseInputForm.vue", () => {
  it("maps input fields correctly to the UI", async () => {
    ...
    expect(getByRole("textbox", { name: "Trainer" })).toHaveValue("Trainer");
    ...
    expect(getByRole("combobox", { name: "Veranstaltungsart" })).toHaveValue(
      "EXTERNAL"
    );
    expect(getByRole("combobox", { name: "Ort" })).toHaveValue("REMOTE");
    expect(getByRole("textbox", { name: "Veranstaltungsadresse" })).toHaveValue(
      "Daheim"
    );
    expect(getByRole("textbox", { name: "Weiterführender Link" })).toHaveValue(
      "https://tarent.de"
    );
    expect(getByRole("textbox", { name: "Zielgruppe" })).toHaveValue("Alle");
  });
  ...
  it("calls 'ready' callback with value 'false' when touch was called with validation errors", async () => {
    ...
    await waitFor(() => expect(wrapper.emitted().ready[0]).toEqual([false]));
  });
